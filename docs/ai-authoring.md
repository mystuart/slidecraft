...
5. 全部通过后，过一遍下方「内容自查清单」：核心概念指代全文一致；每个互动题考察的
...
> 来源：真实课件的两次内容 bug 修复提炼——"形式做对了，但口径跑偏"是最常见也最隐蔽的失败。

### 术语口径一致

- [ ] **核心概念的指代全文统一**（同一概念不要有两个叫法；首次定义用最严谨措辞，后续简化但不能矛盾）
- [ ] **正文、代码示例、互动组件**三处措辞口径一致

### 教学点前置

- [ ] 每个互动题考察的概念，学员在本题之前**已经学过**（先讲再考）
- [ ] 概念「引入 → 应用 → 考察」在章节内有序
- [ ] hint 直击具体误解（给反例），不是"想想看"空话

### 互动题三重对齐

- [ ] 考察点 = 概念本身（问"格式约定"就别拿"具体例子"当答案）
- [ ] 答案可接受范围明确（等价集合写全；说明是否区分大小写）
- [ ] 错答反馈给出"为什么"，不止"再想想"
...
- [ ] `node build.js` 成功、无 ⚠
- [ ] 打开产物翻完所有章节，肉眼检查渲染
- [ ] 所有 quiz / fill-blank 自己答一遍
- [ ] 打印预览：侧栏隐藏、答案展开、无浮层残留
- [ ] 贯穿案例在全文的引用一致
