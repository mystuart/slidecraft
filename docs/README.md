...
Slidecraft 项目的文档目录。**纳入 git 管理**——和源码、SPEC、内容一起进版本控制。

## 当前文件

- [geometry-3d-schema.md](./geometry-3d-schema.md) — geometry-3d 组件的字段契约（**v0.2.0**，含 v0.2.0 新增的 `autoRotate` / `autoRotateSpeed`，4 个示例含三角柱 + 辅助线池 + auxLabels + math-step 联动）
...
- [ai-authoring.md](./ai-authoring.md) — **AI 课件创作指南**（可直接投喂给任意 AI agent：硬约束 + 组件最小 schema + 验证闭环 + 内容自查清单）
- [review-checklist.md](./review-checklist.md) — **复审协议**（两轮真实复审提炼的可重复清单，每项对应实际抓到过的 bug）
