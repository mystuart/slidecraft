...
Slidecraft v1.3.0 新增 4 个 2D 组件：坐标系 + 函数图像 + 交点 + 滑块联动。
**目标用户**：数学家教老师（不是 DBA！框架是通用培训引擎，跨学科）。
...
> **v0.2.0 评审修复**（GLM-5.1）：抛物线 orientation 4 方向画法 + 4x 缩放（C0-1）、`polyRealRoots` 重根漏掉相切（C1-1）、三次极值分类错（C1-2）、抽 `_geom_utils` 共享（C2-1）、`computeDiffCoeffs` 抽 helper（C2-3）。
