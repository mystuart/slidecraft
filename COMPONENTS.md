...
| 2 | quiz | v0.4.0 | 🟢 打磨完成 | 2026-06-05 | 2026-09-11 |
| 3 | quiz-track | v0.4.0 | 🟢 打磨完成 | 2026-06-05 | 2026-09-11 |
| 4 | fill-blank | v0.3.1 | 🟢 打磨完成 | 2026-06-05 | 2026-08-20 |
...
| 8 | callout | v0.3.0 | 🟢 打磨完成 | 2026-06-05 | 2026-06-05 |
| 9 | formula | v0.2.0 | 🟢 打磨完成 | 2026-06-05 | 2026-06-06 |
| 10 | math-step | v0.2.1 | 🟢 打磨完成 | 2026-06-05 | 2026-06-12 |
| 11 | geometry-3d | v0.2.0 | 🟡 首次可用（多能力迭代中） | 2026-06-08 | 2026-06-17 |
| 12 | slider | v0.1.2 | 🟡 首次可用（form-B 联动 function-plot 已加） | 2026-06-10 | 2026-06-13 |
| 13 | tetra-equiv | v0.1.3 | 🟡 首次可用 | 2026-06-11 | 2026-06-17 |
| 14 | cut-anim | v0.1.3 | 🟡 首次可用 | 2026-06-11 | 2026-06-17 |
| 15 | coords-2d | v0.1.1 | 🟡 首次可用 | 2026-06-13 | 2026-06-17 |
| 16 | function-plot | v0.1.1 | 🟡 首次可用 | 2026-06-13 | 2026-06-17 |
| 17 | intersection-marker | v0.1.1 | 🟡 首次可用 | 2026-06-13 | 2026-06-17 |
| 18 | timeline | v0.1.0 | 🟡 首次可用 | 2026-06-15 | 2026-06-15 |
| 19 | chart | v0.1.0 | 🟡 首次可用 | 2026-06-15 | 2026-06-15 |
| 20 | tabs | v0.1.0 | 🟡 首次可用 | 2026-06-16 | 2026-06-16 |
| 21 | stat-grid | v0.1.0 | 🟡 首次可用 | 2026-06-16 | 2026-06-16 |
| 22 | quote | v0.1.0 | 🟡 首次可用 | 2026-06-16 | 2026-06-16 |
| 23 | diagram | v0.1.0 | 🟡 首次可用 | 2026-06-16 | 2026-06-16 |
| 24 | code-runner | v0.1.1 | 🟡 首次可用 | 2026-06-16 | 2026-06-16 |
| 25 | feedback | v0.1.0 | 🟡 首次可用（👍👎 + 文本反馈单回发作者） | 2026-09-11 | 2026-09-11 |
| 25 | trajectory | v0.1.1 | 🟡 首次可用 | 2026-06-11 | 2026-06-17 |
| 26 | _highlight | v0.1.0 | 🟡 内部工具（编译时语法高亮，marked/code-runner 共用） | 2026-06-17 | 2026-06-17 |
| 27 | _geom_utils | v0.2.0 | 🟡 内部工具（polyEval/Deriv/RealRoots + tetra 等） | 2026-06-11 | 2026-06-13 |
| 27 | _inline | v0.2.3 | 🟡 内部工具（LaTeX 行内/块级 + HTML 转义） | 2026-06-05 | 2026-06-17 |
| 28 | _lifecycle | v0.1.0 | 🟡 内部工具（组件生命周期基础设施） | 2026-06-17 | 2026-06-17 |
| 29 | _progress | v0.1.0 | 🟡 内部工具（学习进度持久化 localStorage） | 2026-08-20 | 2026-08-20 |
...
- **方向**：多选状态机（未选 / 部分选 / 全选错 / 全选对 / 提交后）的颜色规范 · feedback 折叠/展开默认决策。
- **v0.3.0（2026-08-20）**：接入 `_progress` 进度持久化（作答/重做，重放式恢复）；build 期硬校验（correct ⊆ options、correct 非空、选项 ≥2，throw 自描述错误）。
- **v0.3.2（2026-09-10）**：**修复属性泄漏 bug**——feedback 含 `$...$` 时 build 后置公式替换把 KaTeX HTML 注进 data-属性撑破泄漏为正文；feedback 改走隐藏 DOM + processInline（解除 LaTeX 限制，系统级问题 #1 收尾）；string 型 feedback 明确为答对文案。
- **v0.3.3（2026-09-11）**：「复制成绩」改调框架 `__SCCopy`，删除组件内独立的 fallback 拷贝（复审去重）。

#### 3. quiz-track *(v0.3.3 已打磨)*
- **当前**：题组 carousel，dots hairline 时间轴 + 4 态节点 + 完成态 4-stat 总结。
- **v0.3.0（2026-08-20）**：active/summary 持久化（key 由组内题目 id 派生，内容变更自动失效）；完成态新增「📋 复制成绩」按钮（文本成绩单一键回发老师）。
- **v0.3.3（2026-09-11）**：复制实现去重——改调框架 `__SCCopy`，与代码卡片复制同一链路。
- **方向**：答对自动跳下一题（流畅感 vs 允许回头看）· 移动端 prev/next 文字可改纯箭头。

#### 4. fill-blank *(v0.3.1 已打磨)*
- **当前**：v0.2.0 多空 `{{1}}` 占位 + 每空独立等价集合；v0.2.1 占位编号硬校验；v0.3.0 进度持久化（输入值实时保存 + 判分态重放恢复）；**v0.3.1 修复无占位单空静默空转**（题面无 `{{n}}` 时整题视为一个空、输入框附题面末尾——binary-card-trick / how-to-create-skill 共 4 道题受益；answers 缺失 throw）。
...
#### 18. geometry-3d *(v0.1.9)*
- **当前**：v0.1.7 已有 30+ 字段（vertices / planes / auxLines / derivedVertices / rightAngles / id），9 种几何体（triangular-prism v0.1.2 真正实现），触摸板 / Z 轴自转 / camera.up=+Z 校准 / per-instance 闭包 / 三角面 / math-step 联动。v0.1.9 接入生命周期基础设施。
...
#### 19-21. slider / tetra-equiv / cut-anim *(v0.1)*
- 三个 3D 联动组件，字段契约见 `docs/slider-schema.md` / `docs/tetra-equiv-schema.md` / `docs/cut-anim-schema.md`。tetra-equiv / cut-anim 已在 v0.1.3 接入生命周期基础设施。
...
- ~~**架构债 C2-4/5 + H4：组件无 destroy、document/window 监听 + 永续 RAF + ResizeObserver 不释放**~~ —— ✅ **2026-06-17 已修复**：引入 `_lifecycle.js` 基础设施（`createLifecycle` 句柄 + 全局 `sc:destroy` 批量销毁），8 个泄漏组件全部接入。见决策记录 #13。
...
1. **~~`processInline` / `processInlineFormulas` 的 `$...$` 正则不识别 `$$...$$` display 分隔符~~**（✅ 2026-06-17 已修复）— 原正则把 `$$S_n=...$$。中文，$O(1)$` 串成跨段匹配，中文喂进 KaTeX 触发 `unicodeTextInMathMode` stderr 告警 + 垃圾 katex-error span。**修复**：先剥 `$$...$$` 占位保护再匹配行内；KaTeX 调用统一加 `strict: 'ignore'`（与 formula.js 对齐）。`_inline.js` v0.2.2 → v0.2.3。
...
9. **~~build.js CSS 注入 4 处重复条件块~~（✅ 2026-06-17 已重构）** — 4 个 `if(bodyHtml.includes('class="X"')) readFileSync` 块 + 一串平行三元拼接，新增组件 CSS 要改 3 处。**修复**：抽 `COMPONENT_CSS` 清单表，循环加载 + join 拼接，单一同步点。
...
### 13. 组件生命周期基础设施（2026-06-17）

全仓原本零 `removeEventListener` / 零 `disconnect` / 零 `cancelAnimationFrame`。8 个组件在 document/window 上挂监听、开永续 RAF 循环、建 ResizeObserver 都不释放。单页课件从不销毁组件所以不炸，但 SPA 嵌入 / 热重载 / 多实例 / 内存分析会泄漏（架构债 C2-4/5 + H4）。

**为什么这样选**：新建 `_lifecycle.js`（内部工具）—— `createLifecycle(root)` 返回 per-element 句柄，统一登记 doc/win 监听 / observer / raf / timeout / 自定义 disposer；`destroy()` 一次性回滚，幂等；`_destroyed` 守卫防 destroy 后再登记。全局注册表 `__SC_LIFECYCLES` + `sc:destroy` 事件支持批量销毁。renderer.collectClientScript 把 runtime 作为所有组件 clientJs 最前缀注入。

**关键约束**：(a) destroy 仅在显式 `sc:destroy` 事件触发时跑，现有单页课件不触发 = **行为零变化**。(b) 匿名 handler 全部改具名才能 removeEventListener。(c) 4 个 3D 组件额外登记 WebGL 资源释放 disposer（OrbitControls.dispose / geometry+material.dispose / renderer.dispose）。

**重评条件**：如果将来组件要在运行时动态增删（不只是 init 一次），需要把 `createLifecycle` 升级为支持 re-init 的模式。但目前所有组件都是 init-once。
...
**跨组件**：
- [x] **组件生命周期基础设施 + 8 组件接入**（架构债 C2-4/5 + H4）— 2026-06-17 完成
- [x] **KaTeX CJK 告警 + `$...$` 错配修复**（`_inline.js` v0.2.3）— 2026-06-17 完成
- [x] **build.js CSS 注入表驱动重构**（消除 4 处重复）— 2026-06-17 完成
- [x] **processInline 升级支持 LaTeX**（影响 7 个组件）— 已完成（架构债 #2 build 期告警 + strict:'ignore'）
- [ ] **CTA 锚点规范**（影响 hero）
