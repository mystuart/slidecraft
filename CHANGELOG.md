...
### 文档各司其职（审计驱动的瘦身：2903 → 2523 行）

对全部 11 份文档做冗余/陈旧审计后重组，原则：**每类信息只有一个家**。

- **SPEC.md 重写为纯架构文档**（570 → 158 行）——§4.1–4.16 约 350 行组件字段契约
  删除（归还各组件 JSDoc，兑现"信息源唯一化"）；新增编译管线图与校验清单表；
  删除 §5《如何创作 SKILL》课件大纲（content/ 里已有真实课件）与 §7 实施分工
  （历史记录）；§2 文件树更新到现实（test/scripts/.github/AGENTS.md）；
  §9 内容自查清单迁往 ai-authoring.md（课件创作指南的正确归属）
- **计数残留修正**：README 竞品表与路线图"25 个"、README.en 标题与表格
  "25 built-in"、AGENTS "25+ 组件 / 84+ 测试" → 26 / 26 / 26 / 90+
- **README 对 SPEC 的描述更新**（"内容大纲"已不存在）；SPEC §4.10 死链改指
  COMPONENTS.md 登记簿
- **docs/README.md 索引补全**（review-checklist 此前漏登记）
- **schema 文档版本头对齐**（slider v0.1.2 / tetra-equiv v0.1.3 / cut-anim v0.1.3）
- **check-registry 扩展**：docs/*-schema.md 版本头纳入守门（负向验证：
  破坏版本头 → exit 1），schema 文档不再静默腐烂
