...
> ```bash
> npm run build:three    # 一次性 esbuild 打包 Three.js → three-bundle.iife.js
> ```
...
1. 在 `template/components/` 下新建 `my-component.js`，导出一个 `render(json) => htmlString` 函数
...
当前 18 个组件之外的候选（与 [SPEC.md §4.10](./SPEC.md#410-组件清单) 同步）：
